...
- ✅ **🧹 Гибридная очистка** - автоматическая очистка после парсинга + ручная очистка
- ✅ **🌍 Мультистрановой парсинг** - поддержка разных стран (UA, GB)
- ✅ **⚙️ Гибкая конфигурация** - включение/отключение сайтов
...
- `country_code` - код страны для ScraperAPI (ua, gb, us, etc.)
- `enabled` - включить/отключить парсинг сайта (true/false)
...
├── clean_products.py    # Утилита очистки товаров
...
### Автоматическая очистка товаров
Проект включает **гибридный подход** к очистке данных:

#### 🔄 Автоматическая очистка (рекомендуется)
Очистка происходит **автоматически после каждого парсинга**:

```bash
# Парсинг + автоматическая очистка
python3 main.py
```

#### 🛠️ Ручная очистка
Для очистки существующих файлов:

```bash
# Очистить все файлы с товарами
python3 clean_products.py
```

### Что очищается:
- **Артикулы в скобках** - `(5099873045367_5099873090442)`
- **Лишние пробелы** - множественные пробелы
- **Пустые строки** - автоматически удаляются

### Примеры очистки:
**До очистки:**
```
Теннессі Віскі Jack Daniel's Old No.7 1 л 40% (5099873045367_5099873090442_082184090442M_3099873045369)
```

**После очистки:**
```
Теннессі Віскі Jack Daniel's Old No.7 1 л 40%
```

### Безопасность:
- ✅ Сохраняет основное название товара
- ✅ Не удаляет важную информацию
- ✅ Работает только с файлами в `output/`
- ✅ Автоматически находит все `.txt` файлы
